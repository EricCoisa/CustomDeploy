    ...
    public class IISManagementService
    {
        ...
        public async Task<PermissionCheckResult> RequestPermissionsAsync()
        {
            ...
            try
            {
                ...
                // 4. Testar acesso ao FileManager (sistema de arquivos)
                _logger.LogDebug("Testando permissões do FileManager");
                try
                {
                    // Testar acesso ao disco C:\
                    var rootAccess = Directory.Exists(@"C:\");
                    
                    if (rootAccess)
                    {
                        // Testar listagem de diretórios na raiz
                        var rootDirectories = Directory.GetDirectories(@"C:\");
                        result.TestDetails.Add($"✅ FileManager: Pode acessar disco C:\\ ({rootDirectories.Length} diretórios encontrados)");
                        
                        // Testar acesso a uma pasta comum
                        if (Directory.Exists(@"C:\Users"))
                        {
                            result.TestDetails.Add("✅ FileManager: Pode acessar C:\\Users");
                        }
                        
                        if (Directory.Exists(@"C:\Program Files"))
                        {
                            result.TestDetails.Add("✅ FileManager: Pode acessar C:\\Program Files");
                        }
                    }
                    else
                    {
                        result.TestDetails.Add("❌ FileManager: Não pode acessar disco C:\\");
                        result.Instructions.Add("• Execute a aplicação como Administrador para acesso completo ao sistema de arquivos");
                    }
                }
                catch (UnauthorizedAccessException ex)
                {
                    result.TestDetails.Add("❌ FileManager: Acesso negado ao sistema de arquivos");
                    result.Instructions.Add("• Execute a aplicação como Administrador para usar o FileManager");
                    _logger.LogWarning(ex, "Teste FileManager: FALHA - Acesso negado");
                }
                catch (Exception ex)
                {
                    result.TestDetails.Add($"❌ FileManager: Erro ao acessar sistema de arquivos: {ex.Message}");
                    _logger.LogWarning(ex, "Teste FileManager: FALHA");
                }

                // 5. Adicionar instruções gerais se houver falhas
...
";
                ...
                try
                {
                    ...
                    if (!appsResult.Success)
                    {
                        return new IISOperationResult
                        {
                            ...
                            Errors = { "Erro ao executar comando PowerShell" }
                        };
                    }
                    ...
                    if (string.IsNullOrWhiteSpace(appsResult.Output))
                    {
                        ...
                        {
                            Success = true,
                            Message = "Nenhuma aplicação encontrada",
                    ...
                    try
                    {
                        ...
                        {
                            Success = true,
                            Message = $"Encontradas {applicationsList.Count} aplicações",
                    ...
                    catch (System.Text.Json.JsonException ex)
                    {
                        ...
                        {
                            Success = false,
                            Message = "Erro ao processar resposta JSON das aplicações",
                            Errors = { ex.Message }
                        };
                    }
                }
            ...
            catch (Exception ex)
            {
                ...
                {
                    Success = false,
                    Message = "Erro interno ao listar aplicações",
                    Errors = { ex.Message }
                };
            }
        }
        ...
        public async Task<IISOperationResult> CreateSiteAsync(CreateSiteRequest request)
        {
            ...
            try
            {
                ...
                if (existingSite != null)
                {
                    ...
                    {
                        Success = false,
                        Message = $"Site '{request.SiteName}' já existe",
                        Errors = { "Site com este nome já foi criado" }
                    };
                }
                ...
                if (portInUse)
                {
                    ...
                    {
                        Success = false,
                        Message = $"Porta especificada já está em uso",
                        Errors = { "A porta do binding já está sendo utilizada por outro site" }
                    };
                }

                // Criar diretório físico se não existir
                if (!Directory.Exists(request.PhysicalPath))
                {
                    Directory.CreateDirectory(request.PhysicalPath);
                    _logger.LogInformation("Diretório físico criado: {PhysicalPath}", request.PhysicalPath);
                }
                ...
                if (appPool == null)
                {
                    // Criar Application Pool se não existir
                ...
                // Criar o site
                var portString = ExtractPortFromBinding(request.BindingInformation);
                var port = int.TryParse(portString, out var parsedPort) ? parsedPort : 80;
                var site = serverManager.Sites.Add(request.SiteName, request.PhysicalPath, port);
                site.Applications[0].ApplicationPoolName = request.AppPoolName;

                // Configurar binding personalizado se necessário
            ...
            try
            {
                ...
                if (site == null)
                {
                    return new IISOperationResult
                    {
                        ...
                        Errors = { "Site especificado não existe" }
                    };
                }
        ...
        public async Task<DetailedSiteInfo?> GetDetailedSiteInfoAsync(string siteName)
        {
            ...
            {
                _logger.LogError(ex, "Erro ao obter informações detalhadas do site: {SiteName}", siteName);
                return null;
            }
        }
        ...
        /// <summary>
        /// Adiciona uma aplicação a um site existente
        /// </summary>
        public async Task<IISOperationResult> CreateApplicationAsync(CreateApplicationRequest request)
        {
            _logger.LogInformation("Criando aplicação: {SiteName}/{AppPath}", request.SiteName, request.AppPath);
            ...
            try
            {
                ...
                if (site == null)
                {
                    ...
                    {
                        Success = false,
                        Message = $"Site '{request.SiteName}' não encontrado",
                        Errors = { "Site especificado não existe" }
                    };
                }
                ...
                if (existingApp != null)
                {
                    ...
                    {
                        Success = false,
                        Message = $"Aplicação '{request.AppPath}' já existe no site '{request.SiteName}'",
                        Errors = { "Aplicação com este caminho já foi criada" }
                    };
                }
                ...
                if (appPool == null)
                {
                    ...
                    {
                        Success = false,
                        Message = $"Application Pool '{request.AppPoolName}' não encontrado",
                        Errors = { "Application Pool especificado não existe" }
                    };
                }
                ...
                _logger.LogInformation("Aplicação criada com sucesso: {SiteName}/{AppPath}", request.SiteName, request.AppPath);

                return new IISOperationResult
                {
                    Success = true,
                    Message = $"Aplicação '{request.AppPath}' criada com sucesso no site '{request.SiteName}'",
            ...
            catch (Exception ex)
            {
                ...
                {
                    Success = false,
                    Message = "Erro interno ao criar aplicação",
                    Errors = { ex.Message }
                };
            }
        }
        ...
        {
            _logger.LogInformation("Atualizando aplicação: {SiteName}/{AppPath}", siteName, appPath);
            ...
            try
            {
                ...
                if (application == null)
                {
                    return new IISOperationResult
                    {
                        ...
                        Errors = { "Aplicação especificada não existe" }
                    };
                }
                ...
                _logger.LogInformation("Aplicação atualizada com sucesso: {SiteName}/{AppPath}", siteName, appPath);

                return new IISOperationResult
                {
                    Success = true,
                    Message = $"Aplicação '{appPath}' atualizada com sucesso",
                    Data = new ApplicationInfo
                    {
            ...
            catch (Exception ex)
            {
                ...
                {
                    Success = false,
                    Message = "Erro interno ao atualizar aplicação",
                    Errors = { ex.Message }
                };
            }
        }
        ...
        {
            _logger.LogInformation("Removendo aplicação: {SiteName}/{AppPath}", siteName, appPath);
            ...
            try
            {
                ...
                // Não permitir remoção da aplicação raiz
                if (appPath == "/")
                {
                    return new IISOperationResult
                    {
                        Success = false,
                        Message = "Não é possível remover a aplicação raiz do site",
                        Errors = { "A aplicação raiz (/) não pode ser removida" }
                    };
                }
                ...
                _logger.LogInformation("Aplicação removida com sucesso: {SiteName}/{AppPath}", siteName, appPath);

                return new IISOperationResult
                {
                    Success = true,
                    Message = $"Aplicação '{appPath}' removida com sucesso do site '{siteName}'"
                };
            }
            catch (Exception ex)
            {
        ...
        public async Task<IISOperationResult> CreateAppPoolAsync(CreateAppPoolRequest request)
        {
            ...
            try
            {
                ...
                // Verificar se o Application Pool já existe
                ...
                if (existingPool != null)
                {
                    ...
                    {
                        Success = false,
                        Message = $"Application Pool '{request.PoolName}' já existe",
                        Errors = { "Application Pool com este nome já foi criado" }
                    };
                }
        ...
        public async Task<IISOperationResult> UpdateAppPoolAsync(string poolName, UpdateAppPoolRequest request)
        {
            ...
            try
            {
                ...
                // Atualizar outras configurações se fornecidas
        ...
        public async Task<IISOperationResult> DeleteAppPoolAsync(string poolName)
        {
            ...
            try
            {
                ...
                // Verificar se está sendo usado por algum site ou aplicação
                ...
                if (usedBy.Any())
                {
                    ...
                    {
                        Success = false,
                        Message = $"Application Pool '{poolName}' está sendo usado e não pode ser removido",
        ...
        /// <summary>
        /// Lista todos os Application Pools com informações detalhadas
        /// </summary>
        public async Task<IISOperationResult> GetAllAppPoolsAsync()
        {
            ...
            try
            {
                ...
                foreach (var appPool in serverManager.ApplicationPools)
                {
                    ...
                    // Encontrar sites e aplicações que usam este pool
        ...
        /// <summary>
        /// Extrai a porta de uma string de binding
        /// </summary>
        private string ExtractPortFromBinding(string bindingInformation)
        {
            try
            {
                // Formato típico: "*:80:" ou "192.168.1.1:8080:"
                var parts = bindingInformation.Split(':');
                if (parts.Length >= 2)
                {
                    return parts[1];
                }
                return "80"; // Porta padrão
            }
            catch
            {
                return "80"; // Porta padrão em caso de erro
            }
        }
